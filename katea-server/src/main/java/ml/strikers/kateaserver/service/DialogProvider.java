...
public class DialogProvider {
    ...
    public Map<String, QueryResult> detectIntentTexts(PassthroughRequest passthroughRequest) throws Exception {
        ...
        try (SessionsClient sessionsClient = SessionsClient.create(sessionsSettings)) {
            UUID sessionId = Objects.isNull(passthroughRequest.getSessionId()) ? UUID.randomUUID() : passthroughRequest.getSessionId();
            final SessionName session = SessionName.of(projectId, sessionId.toString());
            TextInput.Builder textInput = TextInput.newBuilder().setText(passthroughRequest.getMessage()).setLanguageCode(languageCode);
            ...
            queryResults.put(passthroughRequest.getMessage(), queryResult);
        }
        return queryResults;
    }
}
